...
async function fetchAndDisplaySuppliers() {
  ...
  const response = await fetch("/suppliers/demotable", { method: "GET" });
  ...
  demotableContent.forEach((supplier) => {
      ...
      ["supplier_id", "supplier_name", "supplier_address", "supplier_tel"].forEach((field) => {
...
async function resetDemotable() {
  const response = await fetch("/suppliers/initiate-demotable", { method: "POST" });
  const responseData = await response.json();

  document.getElementById("resetResultMsg").textContent = responseData.success
      ? "Supplier table initialized successfully!"
